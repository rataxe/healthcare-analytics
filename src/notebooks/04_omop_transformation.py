...
# ── CELL 3: Skapa person_id-mappning (OMOP kräver INTEGER person_id) ────────
...
# ── CELL 6: VISIT_OCCURRENCE-tabell ─────────────────────────────────────────
...
# ── CELL 7: CONDITION_OCCURRENCE-tabell ──────────────────────────────────────
...
# Materialize the join to break deep query plan (avoids Spark optimizer INTERNAL_ERROR)
_cond_base = (
    diagnoses
    .join(
...
# Materialize the base join ONCE to break deep query plan (avoids 12x Spark optimizer bug)
_meas_base = (
    vitals
    .join(
        encounters.select("encounter_id", "patient_id"),
...
# ── CELL 11: OBSERVATION_PERIOD-tabell ───────────────────────────────────────
...
# ── CELL 12: CONCEPT-tabell (stub/referens-vocabulary) ───────────────────────
